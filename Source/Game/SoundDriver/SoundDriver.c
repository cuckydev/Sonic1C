...
#define SOUND_DRIVER_NUM_MUSIC_DAC_TRACKS (sizeof(v_snddriver_ram.state.v_music_dac_track) / sizeof(SoundDriverTrack))
#define SOUND_DRIVER_NUM_MUSIC_FM_TRACKS (sizeof(v_snddriver_ram.state.v_music_fm1_track) / sizeof(SoundDriverTrack))
#define SOUND_DRIVER_NUM_MUSIC_PSG_TRACKS (sizeof(v_snddriver_ram.state.v_music_psg1_track) / sizeof(SoundDriverTrack))

#define SOUND_DRIVER_NUM_SFX_FM_TRACKS (sizeof(v_snddriver_ram.v_sfx_fm3_track) / sizeof(SoundDriverTrack))
#define SOUND_DRIVER_NUM_SFX_PSG_TRACKS (sizeof(v_snddriver_ram.v_sfx_psg1_track) / sizeof(SoundDriverTrack))

#define SOUND_DRIVER_NUM_SPCSFX_FM_TRACKS (sizeof(v_snddriver_ram.v_spcsfx_fm4_track) / sizeof(SoundDriverTrack))
#define SOUND_DRIVER_NUM_SPCSFX_PSG_TRACKS (sizeof(v_snddriver_ram.v_spcsfx_psg3_track) / sizeof(SoundDriverTrack))

#define SOUND_DRIVER_NUM_MUSIC_TRACKS (SOUND_DRIVER_NUM_MUSIC_DAC_TRACKS + SOUND_DRIVER_NUM_MUSIC_FM_TRACKS + SOUND_DRIVER_NUM_MUSIC_PSG_TRACKS)
#define SOUND_DRIVER_NUM_SFX_TRACKS (SOUND_DRIVER_NUM_SFX_FM_TRACKS + SOUND_DRIVER_NUM_SFX_PSG_TRACKS)
#define SOUND_DRIVER_NUM_SPCSFX_TRACKS (SOUND_DRIVER_NUM_SPCSFX_FM_TRACKS + SOUND_DRIVER_NUM_SPCSFX_PSG_TRACKS)

#define SOUND_DRIVER_NUM_SOUND_QUEUES (sizeof(v_snddriver_ram.state.v_soundqueue) / sizeof(v_snddriver_ram.state.v_soundqueue[0]))
...
#define pcmLoopCounterBase(sampleRate, baseCycles) (int)(1 + (Z80_Clock / (sampleRate) - (baseCycles) + (13.0 / 2))/13)
...
static u8 FMDACInitBytes[SOUND_DRIVER_NUM_MUSIC_DAC_TRACKS + SOUND_DRIVER_NUM_MUSIC_FM_TRACKS];
...
void UpdateMusic(void)
{
	...
	if (v_snddriver_ram.state.v_soundqueue[0] || v_snddriver_ram.state.v_soundqueue[1] || v_snddriver_ram.state.v_soundqueue[2])
#else
	if (v_snddriver_ram.state.v_soundqueue[0] || v_snddriver_ram.state.v_soundqueue[1])
#endif
		CycleSoundQueue();
...
static int DACUpdateTrack(SoundDriverTrack *track)
{
	...
	if (!(track->PlaybackControl & PLAYBACKCONTROL_SFX_OVERRIDE))
	{
		...
		if (sample != 0x80)
		{
			if (!(sample & (1 << 3)))
			{
				/* TODO: Send sample to Z80 driver */
			}
			else
			{
				sample -= 0x88;
...
static int Sound_PlayBGM(u8 sound_id)
{
	...
	if (sound_id == bgm_ExtraLife)
	{
		...
		memcpy(&v_snddriver_ram.state_1up, &v_snddriver_ram.state, sizeof(SoundDriverState));
...
static u8 FMDACInitBytes[SOUND_DRIVER_NUM_MUSIC_DAC_TRACKS + SOUND_DRIVER_NUM_MUSIC_FM_TRACKS] = {
	/* first byte is for DAC; then notice the 0, 1, 2 then 4, 5, 6; this is the gap between parts I and II for YM2612 port writes */
	6, 0, 1, 2, 4, 5, 6
};
	...
	do
	{
		...
		else
		{
			...
			if (voice_control == 0xC0)
			{
				...
				PSG_Input((voice_control | 0x1F) ^ (1 << 5));
			}
		}
...
static void Sound_PlaySpecial(u8 sound_id)
{
	...
	v_snddriver_ram.state.v_special_voice_ptr = sound_base + (((u16)sound_p[0] << 8) | sound_p[1]);
	sound_p += 2;
...
static void TempoWait(void)
{
	...
	v_snddriver_ram.state.v_main_tempo_timeout = v_snddriver_ram.state.v_main_tempo;
...
static int SpeedUpMusic(void)
{
	if (!v_snddriver_ram.state.f_1up_playing)
	{
		...
		v_snddriver_ram.state.v_main_tempo_timeout = v_snddriver_ram.state.v_speeduptempo;
		v_snddriver_ram.state.f_speedup = 0x80;
	}
	else
	{
		v_snddriver_ram.state_1up.v_main_tempo = v_snddriver_ram.state_1up.v_speeduptempo;
		v_snddriver_ram.state_1up.v_main_tempo_timeout = v_snddriver_ram.state_1up.v_speeduptempo;
...
static int SlowDownMusic(void)
{
	if (!v_snddriver_ram.state.f_1up_playing)
	{
		...
		v_snddriver_ram.state.v_main_tempo_timeout = v_snddriver_ram.state.v_tempo_mod;
		v_snddriver_ram.state.f_speedup = 0x00;
	}
	else
	{
		v_snddriver_ram.state_1up.v_main_tempo = v_snddriver_ram.state_1up.v_tempo_mod;
		v_snddriver_ram.state_1up.v_main_tempo_timeout = v_snddriver_ram.state_1up.v_tempo_mod;
...
static void PSGSilenceAll(void)
{
	PSG_Input(0x9F); /* Silence PSG 1 */
	PSG_Input(0xBF); /* Silence PSG 2 */
	PSG_Input(0xDF); /* Silence PSG 3 */
	PSG_Input(0xFF); /* Silence noise channel */
}

#define Fmin(a, b) ((a) < (b) ? (a) : (b))
#define F(frequency) (Fmin(0x3FF, (s16)((double)PSG_Sample_Rate / ((frequency) * 2))))

static s16 PSGFrequencies[12 * 5 + 10] = {
	F( 130.98), F( 138.78), F( 146.99), F( 155.79), F( 165.22), F( 174.78), F( 185.19), F( 196.24), F( 207.91), F(   220.63), F( 233.52), F( 247.47),
	F( 261.96), F( 277.56), F( 293.59), F( 311.58), F( 329.97), F( 349.56), F( 370.39), F( 392.49), F( 415.83), F(   440.39), F( 468.03), F( 494.95),
	F( 522.71), F( 556.51), F( 588.73), F( 621.44), F( 661.89), F( 699.12), F( 740.79), F( 782.24), F( 828.59), F(   880.79), F( 932.17), F( 989.91),
	F(1045.42), F(1107.52), F(1177.47), F(1242.89), F(1316.00), F(1398.25), F(1491.47), F(1575.50), F(1669.55), F(  1747.82), F(1864.34), F(1962.46),
	F(2071.49), F(2193.34), F(2330.42), F(2485.78), F(2601.40), F(2796.51), F(2943.69), F(3107.23), F(3290.01), F(  3495.64), F(3608.40), F(3857.25),
	F(4142.98), F(4302.32), F(4660.85), F(4863.50), F(5084.56), F(5326.69), F(5887.39), F(6214.47), F(6580.02), F(223721.56)
};
...
static void SetVoice(SoundDriverTrack *track, u8 voice, u8 *voice_ptr)
{
	...
	for (i = 0; i < sizeof(FMInstrumentOperatorTable) / sizeof(FMInstrumentOperatorTable[0]); i++)
	...
	for (i = 0; i < sizeof(FMInstrumentTLTable) / sizeof(FMInstrumentTLTable[0]); i++)
	{
		u8 fm_addr, fm_data;
